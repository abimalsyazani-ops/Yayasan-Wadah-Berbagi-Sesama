...
create table if not exists public.volunteers (id text primary key, name text, phone text, email text, city text, role text, availability text, motivation text, message text, status text default 'pending_follow_up', "statusLabel" text default 'Menunggu Follow-up', "confirmedAt" timestamptz, "createdAt" timestamptz default now(), "updatedAt" timestamptz);
create table if not exists public.book_donations (id text primary key, name text, phone text, email text, city text, "bookCount" integer, "bookType" text, "pickupAddress" text, message text, status text default 'pending_follow_up', "statusLabel" text default 'Menunggu Follow-up', "confirmedAt" timestamptz, "createdAt" timestamptz default now(), "updatedAt" timestamptz);
...
create table if not exists public.messages (id text primary key, name text, email text, phone text, subject text, message text, status text default 'pending_follow_up', "statusLabel" text default 'Menunggu Follow-up', "confirmedAt" timestamptz, "createdAt" timestamptz default now(), "updatedAt" timestamptz);
...
grant insert on public.book_donations to anon, authenticated;
grant select, update, delete on public.book_donations to authenticated;
...
create policy "Public insert book donations" on public.book_donations for insert to anon, authenticated with check (true);
drop policy if exists "Authenticated manage book donations" on public.book_donations;
create policy "Authenticated manage book donations" on public.book_donations for all to authenticated using (true) with check (true);
