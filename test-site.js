...
check("Token admin dan fallback publishable dibedakan", store.includes("if(this.accessToken)headers.Authorization='Bearer '+this.accessToken") && store.includes("else if(publicBearer)headers.Authorization='Bearer '+this.key") && !store.includes("this.accessToken||this.key"));
check("Fallback publishable hanya dipakai setelah 401 anonim", store.includes("response.status===401&&!this.accessToken") && store.includes("this.headers(extra,true)"));
check("PWA admin tidak mencache seluruh website", adminSw.includes("wbs-admin-pwa-v16") && adminSw.includes("ADMIN_URLS.has(event.request.url)") && adminSw.includes("cache: 'reload'"));
check("Semua halaman memakai data-store berversi", appPages.every(html => html.includes("assets/data-store.js?v=20260924-1")));
