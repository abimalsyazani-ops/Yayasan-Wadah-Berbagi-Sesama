(function(){
  ...
  class SupabaseRestSync{
    ...
    async hydrate(keys){const results=await Promise.allSettled(this.tables.map(async type=>{const rows=await this.list(type);if(Array.isArray(rows)){const local=safeParse(localStorage.getItem(keys[type]),[]),merged=[...rows,...local.filter(item=>!rows.some(remote=>remote.id===item.id))];localStorage.setItem(keys[type],JSON.stringify(merged));notifySync(type)}}));return results}
  }
  ...
  class LocalRepository{
    ...
    custom(type){normalizeType(type);const retired=retiredSeedIds[type];return safeParse(localStorage.getItem(keys[type])||'[]',[]).filter(item=>!retired?.has(item.id))}
    ...
    async saveAndSync(type,item){const saved=this.save(type,item,{sync:false});try{await supabaseSync.upsert(type,saved);return saved}catch(error){const rows=this.custom(type).filter(row=>row.id!==saved.id);writeRows(type,rows);notifySync(type);throw new Error('Data belum tersimpan ke server. Periksa koneksi lalu coba lagi. ('+error.message+')')}}
