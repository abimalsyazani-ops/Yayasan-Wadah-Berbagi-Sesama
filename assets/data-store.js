(function(){
  const seed={
    ...
    gallery:[
      {id:'gal-1',title:'Distribusi Paket Pangan',category:'Pangan',date:'2026-06-12',image:'https://images.unsplash.com/photo-1593113646773-028c64a8f1b8?w=1000&q=80'},
      {id:'gal-2',title:'Kegiatan Belajar Anak',category:'Pendidikan',date:'2026-06-06',image:'https://images.unsplash.com/photo-1509062522246-3755977927d7?w=1000&q=80'},
      {id:'gal-3',title:'Pemeriksaan Kesehatan',category:'Kesehatan',date:'2026-05-28',image:'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=1000&q=80'},
      {id:'gal-4',title:'Pembinaan Anak Yatim',category:'Sosial',date:'2026-05-20',image:'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=1000&q=80'},
  ...
  class LocalRepository{
    ...
    save(type,item){const rows=this.custom(type);const index=rows.findIndex(row=>row.id===item.id);if(index>=0)rows[index]=item;else rows.unshift(item);localStorage.setItem(keys[type],JSON.stringify(rows));return item}
    remove(type,id){localStorage.setItem(keys[type],JSON.stringify(this.custom(type).filter(item=>item.id!==id)))}
  ...
  window.WBS={seed,keys,repository:new LocalRepository(),LocalRepository,SupabaseRepository,uid(prefix){return prefix+'-'+Date.now()+'-'+Math.random().toString(16).slice(2,8)}};
})();
